...
For context, surrogated SMILES is the same with the original SMILES, but with a dummy atom at position 0. Here is an example how it works for dimetyl ether; To replace the lone electron pair on oxygen with a surrogate  atom, we must "trick" the valence of the oxygen atom and rearrange the SMILES formula so that the marker atom appears first (for easier bookkeeping). - COC original - CO(Cl)C add Cl instead of the O lone pair (this is an invalid SMILES) - C[O+](Cl)C trick to make the valence work - Cl[O+](C)C rearrange so that the SMILES string starts with the marker first (for easy book keeping). You should use the surrogate atom with the provided tools to place it on the surface. When the tool places the ligand on the surface, it will automatically remove the surrogate atom Cl.
